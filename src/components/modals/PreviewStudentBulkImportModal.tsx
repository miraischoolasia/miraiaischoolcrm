...
import {
  buildPreviewStudentCsvTemplate,
  parsePreviewStudentCsv,
  type BulkPreviewStudentRow,
} from '../../lib/studentCsv'

type PreviewStudentBulkImportModalProps = {
  isImporting: boolean
  importError: string | null
  onClose: () => void
  onImport: (rows: BulkPreviewStudentRow[]) => void
}

export function PreviewStudentBulkImportModal({
  isImporting,
  importError,
  onClose,
  onImport,
}: PreviewStudentBulkImportModalProps) {
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [fileName, setFileName] = useState<string | null>(null)
  const [parsedRows, setParsedRows] = useState<BulkPreviewStudentRow[]>([])
  const [parseErrors, setParseErrors] = useState<string[]>([])
  const [readError, setReadError] = useState<string | null>(null)

  function downloadTemplate() {
    const csv = buildPreviewStudentCsvTemplate()
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'preview-student-bulk-import-example.csv'
  ...
  async function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { rows, errors } = parsePreviewStudentCsv(text)
      setParsedRows(rows)
    ...
    <ModalShell maxWidth="2xl" onClose={onClose}>
      <div className="border-b border-slate-200 bg-white px-6 py-5 sm:px-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="text-sm font-medium text-[#be185d]">Preview Class</div>
            <h2 className="mt-1 text-2xl font-semibold text-slate-900">
              Bulk Import Preview Students
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              Upload a CSV file with names and phone numbers only.
            </p>
          </div>
          <button
            type="button"
      ...
      <div className="max-h-[82vh] space-y-6 overflow-y-auto px-6 py-6 sm:px-8">
        ...
        <div className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Fill one row per preview student.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Required columns: Student Name, Phone.
            </p>
          </div>
          ...
          {fileName && !readError && (
            <div className="space-y-2 rounded-xl border border-slate-200 bg-white p-3">
              <p className="text-sm font-medium text-slate-700">
                {parsedRows.length} preview student
              ...
              {parseErrors.length > 0 && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <p className="text-xs font-semibold text-amber-800">
                    ...
                    {parseErrors.length === 1 ? '' : 's'} skipped or need fixing:
                  </p>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:items-center sm:justify-end">
          ...
          >
            {isImporting
              ? 'Importing...'
              : `Import ${parsedRows.length || ''} Preview Student${
